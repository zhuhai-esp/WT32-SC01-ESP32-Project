...
public:
  LGFX(void) {
    {
      auto cfg =
          _bus_instance.config(); // Get the structure for bus configuration.
      ...
      cfg.freq_write = 40000000; // SPI clock when sending (up to 80MHz, rounded
...
void setup() {
  ...
  if (!disp_draw_buf) {
    ...
    lv_label_set_text(
        label1,
        "Hello there! You have not included UI files, add you UI files and "
...
void loop() {
  ...
  wukong = btn1; // digitalRead(40);
  Kong = btn2;   // digitalRead(19);
